  ...
  return new Promise((resolve, reject) => {
    ...
    proc.stderr.on('data', (data: Buffer) => {
      ...
      if (mainWindow && duration) {
        ...
        if (timeMatch) {
          ...
          mainWindow.webContents.send(IPC_CHANNELS.EDITOR_PROGRESS, progress);
        }
      }
    });
    ...
    proc.on('close', (code) => {
      if (code === 0) {
        ...
        reject(
          new Error(`ffmpeg failed with code ${code}: ${stderr.slice(-500)}`),
        );
      }
    });
  });
...
export async function extractSubtitle(
  ...
  try {
    ...
    const output =
      outputPath ||
      generateOutputPath(inputPath, `_subtitle_${trackIndex}`, format);

    const args = [
      '-i',
      inputPath,
      '-map',
      `0:s:${trackIndex}`, // Select specific subtitle stream
      '-c:s',
      format === 'srt' ? 'srt' : format === 'ass' ? 'ass' : 'webvtt',
      '-y',
      output,
    ];
...
export async function addSubtitle(
  ...
  try {
    ...
    const args = [
      ...
      '-map',
      '1', // All streams from second input (subtitle)
    ];
...
export async function removeSubtitle(
  ...
  try {
    ...
    const args = [
      ...
      '0',
      '-map',
      `-0:s:${trackIndex}`, // Negative mapping to exclude
      '-y',
      outputPath,
    ];
...
export async function setDefaultSubtitle(
  ...
  try {
    ...
    const args = [
      ...
      '0',
      '-disposition:s',
      '0', // Reset all subtitle dispositions
      `-disposition:s:${trackIndex}`,
      'default', // Set this one as default
      '-y',
      outputPath,
    ];
